...
class Hypercube(Graph):
    ...
    def __init__(self, length: int, dimension: int, pbc: bool = True, next_nearest: bool = False) -> None:
        """
    ...
    def _create_adj_list(self) -> List[List[int]]:
        """
        ...
        for p in range(self.num_points):
            ...
            for d in range(self.dimension):
                ...
                if self.pbc or (not self.pbc and p_coordinate[d] - 1 >= 0):
                    neighbor2[d] = (p_coordinate[d] - 1 +
                                    self.length) % self.length
                    adj_list[p].append(self._coordinate_to_point(neighbor2))

        return adj_list

    def _create_adj_list_next(self) -> List[List[int]]:
        """
            ...
            for p in range(self.num_points):
                ...
                for d in range(self.dimension):
                    ...
                    if self.pbc or (not self.pbc and p_coordinate[d] + 2 < self.length):
                        ...
                        adj_list[p].append(
                            self._coordinate_to_point(neighbor1))
                    if self.pbc or (not self.pbc and p_coordinate[d] - 2 >= 0):
                        neighbor2[d] = (p_coordinate[d] - 2 +
                                        self.length) % self.length
                        adj_list[p].append(
                            self._coordinate_to_point(neighbor2))

        else:
            # Generate all possible next nearest neighbours coordinate
            binary = [format(a, "#0%db" % (self.dimension+2))[2:]
                      for a in range(2 ** self.dimension)]
            array_directions = [
                [1 if b == '1' else -1 for b in bin] for bin in binary]
    ...
    def _find_bonds(self) -> Tuple[int, List[Tuple[int, int]]]:
        """
    ...
    def _find_bonds_next(self) -> Tuple[int, List[Tuple[int, int]]]:
        """
    ...
    def _point_to_coordinate(self, point: int) -> List[int]:
        """
    ...
    def _coordinate_to_point(self, coordinate: List[int]) -> int:
        """
