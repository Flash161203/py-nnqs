...
class MLPRealPosSupervised(MLP):
    ...
    def sample(self, num_samples):
        ...
        samples_index = np.random.choice(
            range(num_conf), num_samples, p=probs.detach().numpy().flatten(), replace=True)
    ...
    def get_all_probability(self):
        ...
        """
        return self.get_wave_function() ** 2 / torch.sum(self.get_wave_function() ** 2)
