...
class LoggerSupervised(object):
    ...
    def make_experiment_logs(self, path):
        ...
        if not os.path.exists(path + 'experiment_logs.csv'):
            with open(path + 'experiment_logs.csv', 'w') as f:
                f.write(
                    'num,ground_energy,ground_energy_std,epoch,time,overlap,loss,observables\n')
                f.close()
    ...
    def calculate_observables(self, learner, num_samples=1000, num_steps=100, num_division=1):
        ...
        ff.write('%.5f\n' % np.std(new_energy))
        ff.close()
        self.observ_str += '%.5f,%.5f' % (np.mean(new_energy),
                                          np.std(new_energy))
        ...
        if len(prob_out) < 5000:
            ...
            pickle.dump((confs, prob_out), open(
                self.result_path + 'probs.p', 'wb'))
    ...
    def visualize_energy(self, learner):
        ...
        if 'eloc_mean' not in learner.history.keys():
            ...
            plt.figure()
            plt.title('Energy vs Iteration, %s, %s' %
    ...
    def visualize_overlap(self, learner):
        ...
        if 'overlap' not in learner.history.keys():
            ...
            plt.figure()
            plt.title('Overlap vs Iteration, %s, %s' %
                      (str(learner.hamiltonian), str(learner.model)))
            plt.ylabel('Overlap')
            plt.xlabel('Iteration #')
    ...
    def visualize_loss(self, learner):
        ...
        """
        plt.figure()
        plt.title('Loss vs Iteration, %s, %s' %
                  (str(learner.hamiltonian), str(learner.model)))
        plt.ylabel('Loss')
        plt.xlabel('Iteration #')
    ...
    def write_logs(self, learner):
        ...
        if 'eloc_mean' in learner.history.keys():
            ...
            if learner.reference_energy is None:
                ...
                rel_errors = np.abs(
                    (ground_energys - learner.reference_energy) / learner.reference_energy)
        ...
        ff = open(path, 'w')
        ff.write(
            'epoch,ground_energy,ground_energy_std,overlaps,losses,rel_error,time\n')
