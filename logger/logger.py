...
class Logger(object):
    ...
    def log(self, learner):
        ...
        if learner.observables is not None and len(learner.observables) > 0:
            self.calculate_observables(
                learner, learner.sampler.num_samples, learner.sampler.num_steps)
    ...
    def calculate_observables(self, learner, num_samples=1000, num_steps=100, num_division=1):
        ...
        new_samples = learner.sampler.sample(
            learner.model, init_samples, int(num_samples / num_division))
    ...
    def write_logs(self, learner):
        ...
        ff = open(path, 'w')
        ff.write(
            'epoch,ground_energy,ground_energy_std,ground_energy_window,rel_error,time\n')
