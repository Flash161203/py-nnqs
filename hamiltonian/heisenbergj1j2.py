...
class HeisenbergJ1J2(Hamiltonian):
    ...
    next nearest neighbor interaction along x-, y-, and z-axis with magnitude J_2,
    ...
    def __init__(self, graph, j1=1.0, delta=1.0, j2=1.0, total_sz=None):
        ...
        Args:
            ...
            delta: magnitude of the nearest neighbor interaction along the z-axis
    ...
    def calculate_hamiltonian_matrix(self, samples, num_samples):
        ...
        for (s, s_2) in self.graph.bonds:
            ...
            if off_diagonal is None:
                off_diagonal = (
                    self.j1 - samples[:, s] * samples[:, s_2]).view(num_samples, 1)
            else:
                temp = (self.j1 - samples[:, s] *
                        samples[:, s_2]).view(num_samples, 1)
                off_diagonal = torch.cat((off_diagonal, temp), dim=1)
        ...
        for (s, s_2) in self.graph.bonds_next:
            ...
            if off_diagonal is None:
                off_diagonal = (
                    self.j2 - samples[:, s] * samples[:, s_2]).view(num_samples, 1)
            else:
                temp = (self.j1 - samples[:, s] *
                        samples[:, s_2]).view(num_samples, 1)
                off_diagonal = torch.cat((off_diagonal, temp), dim=1)
    ...
    def calculate_ratio(self, samples, model, num_samples):
       ...
       \Psi is defined in the model. 
        ...
        Return:
            ...
            In the Heisenberg model, this corresponds to x' where two adjacent spins are flipped. 
            Therefore, the number of columns equals the number of particles + 1 and the number of rows = num_samples
        ...
        for (s, s2) in self.graph.bonds:
            # Flip 2 adjacent spins
            flipped_s = samples[:, s].neg().view(num_samples, 1)
            flipped_s2 = samples[:, s2].neg().view(num_samples, 1)
            if s == 0:
                new_config = torch.cat(
                    (flipped_s, flipped_s2, samples[:, s2 + 1:]), dim=1)
            elif s2 == self.graph.num_points - 1:
                new_config = torch.cat(
                    (samples[:, :s], flipped_s, flipped_s2), dim=1)
            else:
                new_config = torch.cat(
                    (samples[:, :s], flipped_s, flipped_s2, samples[:, s2 + 1:]), dim=1)
        ...
        for (s, s2) in self.graph.bonds_next:
            ...
            middle_conf = samples[:, s + 1].view(num_samples, 1)
            if s == 0:
                new_config = torch.cat(
                    (flipped_s, middle_conf, flipped_s2, samples[:, s2 + 1:]), dim=1)
            elif s2 == self.graph.num_points - 1:
                new_config = torch.cat(
                    (samples[:, :s], flipped_s, middle_conf, flipped_s2), dim=1)
            else:
                new_config = torch.cat(
                    (samples[:, :s], flipped_s, middle_conf, flipped_s2, samples[:, s2 + 1:]), dim=1)
    ...
    def diagonalize(self):
        ...
        # Calculate next-nearest interaction energy
    ...
    def diagonalize_sparse(self):
        ...
        for row in range(num_confs):
            ...
            for (i, j) in self.graph.bonds:
                ...
                col = int(
                    ''.join(['1' if a == 1 else '0' for a in conf_temp]), 2)
                if col == row:
                    continue
                if self.total_sz is not None:
                    if col not in index:
    ...
    def to_xml(self):
        stri = ""
        stri += "<hamiltonian>\n"
        stri += "\t<type>heisenberg j1 j2</type>\n"
        stri += "\t<params>\n"
        stri += "\t\t<j1>%.2f</j1>\n" % self.j1
        stri += "\t\t<j2>%.2f</j2>\n" % self.j2
        stri += "\t\t<delta>%.2f</delta>\n" % self.delta
